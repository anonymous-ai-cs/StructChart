...
class CSVMetric(evaluate.Metric):
    ...
    def _compute(
        ...
        def csv2triples(csv, separator='\\t', delimiter='\\n'):  #这里可以对于仿真数据集，可以按照规则排列一下三元组中前两个的顺序
            lines = csv.strip().split(delimiter)
            header = lines[0].split(separator) #原始是0
            triples = []
            for line in lines[1:]:   #原始是1
                if not line:
                    continue
                values = line.split(separator)
                entity = values[0]
                for i in range(1, len(values)):
                    ...
                    #---------------------------------------------------------
                    temp = sorted([entity.strip(), header[i].strip()])  # 对entity和header[i]按照首字母排序
                    value = values[i].strip()
                    value = value.replace("%","")     #删除value的百分号
                    value = value.replace("$","")     #删除value的美元符号
        ...
        def csv2triples_plotqa(csv, separator='\\t', delimiter='\\n'):  #这里可以对于仿真数据集，可以按照规则排列一下三元组中前两个的顺序
            lines = csv.strip().split(delimiter)
            header = lines[1].split(separator) #原始是0
            triples = []
            for line in lines[2:]:   #原始是1
                if not line:
                    continue
                values = line.split(separator)
                entity = values[0]
        ...
        def get_eval_list(pred_csv, label_csv, separator='\\t', delimiter='\\n', tol_word=3, tol_num=0.05):
            ...
            for it in pred_csv:
                ...
                #pred_triple_temp = csv2triples_plotqa(it, separator=separator, delimiter=delimiter)  #plotqa专属
        ...
        for sim_threhold in np.arange (0.5, 1, 0.05):
            map_temp_strict = get_ap(predictions, labels, sim_threhold=sim_threhold, tolerance='strict', separator=s, delimiter=d)
            map_temp_slight = get_ap(predictions, labels, sim_threhold=sim_threhold, tolerance='slight', separator=s, delimiter=d)
            map_temp_high = get_ap(predictions, labels, sim_threhold=sim_threhold, tolerance='high', separator=s, delimiter=d)
            map_strict += map_temp_strict/10
            map_slight += map_temp_slight/10
            map_high += map_temp_high/10

        em = get_ap(predictions, labels, sim_threhold=1, tolerance='strict', separator=s, delimiter=d)
        ap_50_strict = get_ap(predictions, labels, sim_threhold=0.5, tolerance='strict', separator=s, delimiter=d)
        ap_75_strict = get_ap(predictions, labels, sim_threhold=0.75, tolerance='strict', separator=s, delimiter=d)    
        ap_90_strict = get_ap(predictions, labels, sim_threhold=0.90, tolerance='strict', separator=s, delimiter=d)
        ap_50_slight = get_ap(predictions, labels, sim_threhold=0.5, tolerance='slight', separator=s, delimiter=d)
        ap_75_slight = get_ap(predictions, labels, sim_threhold=0.75, tolerance='slight', separator=s, delimiter=d)    
        ap_90_slight = get_ap(predictions, labels, sim_threhold=0.90, tolerance='slight', separator=s, delimiter=d)
        ap_50_high = get_ap(predictions, labels, sim_threhold=0.5, tolerance='high', separator=s, delimiter=d)
        ap_75_high = get_ap(predictions, labels, sim_threhold=0.75, tolerance='high', separator=s, delimiter=d)    
        ap_90_high = get_ap(predictions, labels, sim_threhold=0.90, tolerance='high', separator=s, delimiter=d)
